"""One-time script to create / update prompts in Langfuse Prompt Management."""
...
# ---------------------------------------------------------------------------
# Prompt texts (same as config.py but without hardcoding in agents)
# ---------------------------------------------------------------------------

PLANNER_PROMPT = """You are a Research Planner. Your job is to analyze a research request and decompose it into a structured research plan.
...
RESEARCHER_PROMPT = """You are a Senior Research Analyst. Execute the provided research plan thoroughly.
...
CRITIC_PROMPT = """You are a Research Critic. Independently verify and evaluate research findings.
...
SUPERVISOR_PROMPT = """You are a Research Supervisor. You MUST coordinate ALL four steps of the research pipeline. NEVER stop early.
...
PROMPTS = [
    ("mas-planner", PLANNER_PROMPT),
    ("mas-researcher", RESEARCHER_PROMPT),
    ("mas-critic", CRITIC_PROMPT),
    ("mas-supervisor", SUPERVISOR_PROMPT),
]
...
for name, text in PROMPTS:
    try:
        p = lf.create_prompt(
            ...
            prompt=text,
            labels=["production"],
            commit_message="Initial version from homework-12",
        )
        print(f"✅ Created prompt '{name}' v{p.version}")
    except Exception as e:
        print(f"⚠️  '{name}': {e}")

print("\nDone. Check Langfuse UI → Prompts.")
